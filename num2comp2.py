# python num2comp2.py MovieLens1m.csv -o ml1m2 -n 200
# python num2comp2.py ml10m_ratings -o ml10m2 -n 200
# python num2comp2.py netflix -o netflix2 -n 200
...
def num2comp(filename, output, n_train, n_test):
  ...
  for u in xrange(1, n_users+1):
    ...
    if len(ratings_list) >= n_train + n_test:
      ...
      test = ratings_list[:n_test]
      test.sort(cmp=pair_comp)
      train = ratings_list[n_test:]
...
if __name__ == "__main__":
  ...
  parser.add_argument('-n', '--train_items', action='store', dest='n_train', type=int,
                      default=50, help="Minimum number of training items per user (Default 50)") 
  parser.add_argument('-t', '--test_item', action='store', dest='n_test', type=int,
                      default=10, help="Number of test items per user (Default 10)")
